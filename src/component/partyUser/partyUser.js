...
class PartyU extends React.Component {
	...
	render() {
		return ( <
			...
			div style = {
				{ float: 'left', width: 'auto', paddingRight: '10px', marginLeft: '20px' } } > 3000 { /*{this.state.currentPartyData.total}*/ } < /div> <
			div > Baht < /div> <
			/div>
			...
			div > {
				Object.keys(this.state.comingPartyData).map((i) => {
					return ( <
						...
						/> <
						div style = {
							{ float: 'left', width: 'auto', paddingRight: '10px', marginLeft: '20px' }
